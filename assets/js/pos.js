...
searchList.addEventListener('click', function (event) {
    if (event.target.tagName === 'LI') {
        // Verificar si ya hay conceptos en el carrito
        const table = document.querySelector('#carrito tbody');
        const carritoRows = table.querySelectorAll('tr');
        
        if (carritoRows.length > 0) {
            // Si ya hay conceptos en el carrito, mostrar mensaje de error
            Swal.fire({
                icon: 'error',
                title: 'Carrito Lleno',
                text: 'Solo se puede agregar un concepto al carrito. Vacíe el carrito para agregar uno nuevo.',
...
function calculateTotal() {
    let total = 0;
    document.querySelectorAll('#carrito tbody tr').forEach(row => {
        const quantity = 1; // Forzar cantidad a 1
        const price = parseFloat(row.cells[1].textContent);
        const subtotal = quantity * price;
        row.cells[3].textContent = subtotal.toFixed(2);
        total += subtotal;
